...
//  2. Getting all the services for the respective creator
  ...
  const addservice = async (sname,sdesc,ldesc,slug,simg,surl,tags,stype,isPaid,smrp,ssp) => {
      const response = await fetch(`${host}/api/services/createservice`, {
        ...
        body: JSON.stringify({
         sname:sname,sdesc:sdesc,ldesc:ldesc,slug:slug,tags:tags,simg:simg,surl:surl,stype:stype,isPaid:isPaid,smrp:smrp,ssp:ssp
        })
      });
      const json = await response.json();
      return json
  ...
  return (
    <ServiceContext.Provider
      value={{ serviceInfo,services,slugCount,getserviceusingid,getallservicesusingid,getallservices,addservice,deleteService,Uploadfile,getserviceinfo,getslugcount}}
    >
      {" "}
